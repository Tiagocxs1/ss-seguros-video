...
for row in csv1_data:
    ...
    csv1_segments.append({
        "start": row[1].split("–")[0].strip() if "–" in row[1] else row[1].split("-")[0].strip(),
        "end": row[1].split("–")[1].strip() if "–" in row[1] else row[1].split("-")[1].strip(),
        "start_sec": tc_to_seconds(row[1])[0],
        "end_sec": tc_to_seconds(row[1])[1],
        "narracao": row[2],
...
for row in csv2_data:
    ...
    # Find matching CSV1 segment by time overlap
    tc = row[1].replace(" ", "")
    start_sec, end_sec = tc_to_seconds(row[1])
    mid_sec = (tc_to_seconds(row[1])[0] + tc_to_seconds(row[1])[1]) / 2
    
    match = None
    for seg in csv1_segments:
        if seg["start_sec"] <= mid_sec < seg["end_sec"]:
            match = seg
            break
    if match is None:
        match = min(csv1_segments, key=lambda s: abs(s["start_sec"] - mid_sec))
...
for i, row in df_merged.iterrows():
    status = "OK" if row["narracao_exata_tts"] else "NO MATCH"
    print(f"  Row {row['ordem']} ({row['timecode']}): {status} - {row['funcao']}")